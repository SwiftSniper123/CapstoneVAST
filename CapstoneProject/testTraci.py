...
# sumoCmd = [sumoBinary, "-c", "C:/Users/jaron/PycharmProjects/SumoTraci/WorkingDir/sumo.sumocfg"]
# traci.start(sumoCmd)
# traci.vehicle.addFull("veh0","route0")

# traci.start([sumolib.checkBinary('C:/Program Files (x86)/Eclipse/Sumo/bin/sumo-gui.exe'), "-c", "C:/Users/jaron/PycharmProjects/SumoTraci/WorkingDir/my_config_file.sumocfg",
#             '--ignore-route-errors'], 1337)
             #'--vehroute-output', 'vehroutes.xml'])

#traci.start(["sumo-gui", "-c", "C:/Users/jaron/PycharmProjects/SumoTraci/WorkingDir/my_config_file.sumocfg"]) 
...
vehID = "SmartAlec"
#traci.vehicle.add(vehID,"route0")



testObject = traci.vehicle.moveToXY(vehID, edgeID="1to2", lane=0, x=0, y=-2, angle=90, keepRoute=2)
#traci.vehicle.add(vehID2,"route0")
#vehicle = traci.vehicle.moveToXY(vehID2, edgeID="1to2", lane=0, x=80, y=-2, angle=90, keepRoute=2)
...
#	testObject = traci.vehicle.moveToXY(vehID, edgeID="1to2", lane=0, x=i, y=i, angle=90, keepRoute=2)
#    print("Position: ", traci.vehicle.getPosition(vehID))
#	step()

#vehiclesub = traci.vehicle.subscribe(vehID)
i = 0
while(traci.simulation.getTime() != 10):
    testObject = traci.vehicle.moveToXY(vehID, edgeID="1to2", lane=0, x=i, y=-2, angle=90, keepRoute=2)
    i += 0.5
    #vehicle = traci.vehicle.moveToXY(vehID2, edgeID="1to2", lane=0, x=40, y=-7, angle=90, keepRoute=2)
    
    #print("Position: ", traci.vehicle.getPosition(vehID))
    #print("Position: ", traci.vehicle.getPosition(vehID2))
    #print("speed", traci.vehicle.getSpeed(vehID))
